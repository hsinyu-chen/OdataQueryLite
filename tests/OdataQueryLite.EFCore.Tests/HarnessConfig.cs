namespace OdataQueryLite.EFCore.Tests
{
    ...
    /// environment variable: <c>sqlite</c> (default) or <c>localdb</c> / <c>mssql</c> / <c>sqlserver</c>
    /// (SQL Server LocalDB). The golden fixture is provider-specific so an engine run is never compared
    /// against a golden captured on a different backend — SQLite cannot translate date/math functions,
    /// DateTimeOffset ORDER BY, or native decimal that SQL Server can, so the two diverge legitimately.
    /// </summary>
